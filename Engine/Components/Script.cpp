...
namespace eridan::script
{
	namespace {
		...
		using script_register = std::unordered_map<size_t, detail::script_creator>;

		script_register& registery()
		{
			//NOTE (Zaurac): we put this static variable in a function because of
			//				 the initialization order of static data. This way, we can
			//				 be certain that the data is initalized before accesing it.
			static script_register reg;
			return reg;
		}
	...
	namespace detail {
		...
		{
			bool result{registery().insert(script_register::value_type{tag,func}).second};
			assert(result);
			return result;
		}
	}
